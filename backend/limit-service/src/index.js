...
async function applyRequestChaos() {
  ...
  if (chaosState.memoryStressMb > 0) {
    retainedMemory = [Buffer.alloc(chaosState.memoryStressMb * 1024 * 1024, 9)];
  } else {
    retainedMemory = [];
  }

  if (chaosState.mode === "packet_loss" && Math.random() < chaosState.packetLossRate) {
    const error = new Error("Simulated packet loss in limit-service.");
  ...
  if (chaosState.mode === "partial_failure") {
    ...
    if (shouldFail) {
      const error = new Error("Simulated partial failure in limit-service.");
...
app.post("/limits/check", async (req, res) => {
  ...
  try {
    ...
    const approved = amount <= Number(account.dailyTransferLimit || 0);

    return res.json({
      accountId,
      approved,
      remainingLimit: Math.max(0, Number(account.dailyTransferLimit || 0) - Number(amount || 0)),
      dependencyStatus: "UP"
    });
  } catch (error) {
...
app.listen(PORT, () => {
  console.log(`[limit-service] listening on port ${PORT}`);
});
